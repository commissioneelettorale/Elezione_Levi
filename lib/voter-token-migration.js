...
async function migrate({ db, year, migrationId, apply = false, now = () => Date.now(), timestamp = () => new Date(), onProgress = () => {} }) {
  ...
  try {
    const plan = await db.runTransaction(async tx => {
      ...
      if (previous.initialCounts && JSON.stringify(previous.initialCounts) !== JSON.stringify(stats.counts)) stop('REGISTER_COUNT_CHANGED');
      if (apply) {
    ...
    return await db.runTransaction(async tx => {
      ...
      if (stats.remaining || stats.total !== record.initialTotal || JSON.stringify(stats.counts) !== JSON.stringify(record.initialCounts)) stop('FINAL_VERIFICATION_FAILED');
