...
const romeFormat=new Intl.DateTimeFormat('en-GB',{timeZone:'Europe/Rome',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hourCycle:'h23'});
function localDate(iso,time){
 const cacheKey=iso+'|'+time;if(dateCache.has(cacheKey))return new Date(dateCache.get(cacheKey));
 const result=parseLocalDate(iso,time);if(result){if(dateCache.size>=512)dateCache.delete(dateCache.keys().next().value);dateCache.set(cacheKey,+result);}return result;
}
function parseLocalDate(iso,time){
...
function phase(config,key,now=Date.now()){
 if(profile(config,key).dedicated!==true)return'UNCONFIGURED';
...
// Limiti delle preferenze per le consultazioni della scuola secondaria di II grado.
function preferenceLimit(config,key,type){
 const fields={consiglio:'maxPrefConsiglio',istituto:'maxPrefIstituto',consulta:'maxPrefConsulta',classeStudente:'maxPrefClasseStudenti',classeGenitore:'maxPrefClasseGenitori'};
 const ceiling=(key==='consiglio'&&type!=='ATA')||key==='istituto'?2:1;
 const configured=Number(config?.[fields[key]]);
 return Number.isInteger(configured)&&configured>0?Math.min(configured,ceiling):ceiling;
}
return{ELECTIONS,profile,windows,phase,globalPhase,enabled,keyForCollection,localDate,isoDate,validateProfile,preferenceLimit};
});
