(function(root){
 ...
 const download=(content,name,type)=>{const url=URL.createObjectURL(new Blob([content],{type}));const a=document.createElement('a');a.href=url;a.download=name;a.click();setTimeout(()=>URL.revokeObjectURL(url),30000);};
 ...
 async function makePdf(data){
  ...
  for(const section of root.LeviDpoDossier.reviewSections(data)){
 ...
 async function exportDossier(button){
  ...
  try{
   const [review,profile,technical,inventory,development,notice,privacy,procedure]=await Promise.all([
    call('getVotingReview'),call('getDpoReviewProfile'),app.logs(),resource('docs/inventario-release.json'),resource('docs/verifiche-sviluppo.json'),resource('note-legali.html'),resource('anonimato.html'),resource('docs/19_FASCICOLO_DPO_E_RIPRESA.md')]);
   ...
   download(await zip.generateAsync({type:'uint8array',compression:'DEFLATE'}),'Fascicolo_DPO_'+review.year.replace('/','_')+'.zip','application/zip');
   app.notify('Fascicolo DPO creato','PDF, architettura, matrice, rapporti, inventario e impronte inclusi. Custodire la copia e farla valutare; non contiene le urne o i codici di voto.','success');
  }catch(e){app.notify('Fascicolo non esportato',e.message,'error');}finally{button.disabled=false;button.textContent=prior;}
 }
 async function show(){
  let host=document.getElementById('privacy-review-dialog');
  if(!host){host=document.createElement('dialog');host.id='privacy-review-dialog';host.style.cssText='width:min(1080px,96vw);max-height:92vh;overflow:auto;border:1px solid #94a3b8;border-radius:16px;color:#111;background:#fff;padding:24px';document.body.appendChild(host);}
  if(!host.open)host.showModal();host.innerHTML='<p>Caricamento fascicolo e stato dei controlli…</p>';
  try{
   const [view,profile]=await Promise.all([call('getVotingReview'),call('getDpoReviewProfile')]),canEdit=view.role==='COMMISSIONE',st=view.review?.stage;
   ...
   host.querySelector('[data-close]').onclick=()=>host.close();host.querySelector('[data-export]').onclick=e=>exportDossier(e.currentTarget);
   ...
   host.querySelector('[data-profile]').onsubmit=e=>{e.preventDefault();action(e.submitter,()=>call('saveDpoReviewProfile',{fields:Object.fromEntries(new FormData(e.target))}));};
   if(canEdit){
    ...
    host.querySelector('[data-batch]').onsubmit=e=>{e.preventDefault();const values=Object.fromEntries(new FormData(e.target));action(e.submitter,async()=>{
     ...
     const result=await call('createAnonymousCredentials',{...values,count:Number(values.count)}),doc=new root.jspdf.jsPDF();
     result.codes.forEach((code,i)=>{if(!/^[A-F0-9]{32}$/.test(code))throw new Error('Formato credenziale non valido.');if(i&&i%7===0)doc.addPage();const y=20+(i%7)*37;doc.setFontSize(10);doc.text('Primo Levi - '+result.tipo+' '+result.classe,15,y);doc.text(code.match(/.{1,4}/g).join('-'),15,y+8);doc.setFontSize(8);doc.text('Codice non nominativo. Custodire il cedolino. Non annotare il nome.',15,y+16);doc.line(15,y+26,195,y+26);});
     doc.save('Cedolini_non_nominativi_'+result.tipo+'_'+(result.classe||'gruppo')+'.pdf');result.codes.fill('');
    });};
   }
  ...
  }catch(e){host.innerHTML='<p>'+escape(e.message)+'</p><button data-close class="border p-2">Chiudi</button>';host.querySelector('[data-close]').onclick=()=>host.close();}
 }
 root.LeviPrivacyReview={configure:value=>{app=value;},show,makePdf,exportDossier};
})(window);
