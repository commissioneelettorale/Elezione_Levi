  ...
  try{
   ...
   if(canEdit){
    ...
    host.querySelector('[data-batch]').onsubmit=e=>{e.preventDefault();const values=Object.fromEntries(new FormData(e.target));action(e.submitter,async()=>{
     ...
     result.codes.forEach((code,i)=>{if(!/^(?:(?:STU|GEN|DOC|ATA)-[A-Z0-9]{6}|[A-F0-9]{32})$/.test(code))throw new Error('Formato credenziale non valido.');if(i&&i%7===0)doc.addPage();const y=20+(i%7)*37;doc.setFontSize(10);doc.text('Primo Levi - '+result.tipo+' '+result.classe,15,y);if(result.electionKey){doc.setFontSize(8);doc.text(result.electionKey==='consiglio'?'SOLO CONSIGLIO D’ISTITUTO':'SOLO RAPPRESENTANTI DI CLASSE',15,y+4);doc.setFontSize(10);}doc.text(code.includes('-')?code:code.match(/.{1,4}/g).join('-'),15,y+8);doc.setFontSize(8);doc.text('Codice non nominativo. Custodire il cedolino. Non annotare il nome.',15,y+16);doc.line(15,y+26,195,y+26);});
