(function(root){
 ...
 async function exportDossier(button,format='zip'){
  ...
  try{
   ...
   if(format==='pdf'){download(pdf,'Fascicolo_DPO_'+review.year.replace('/','_')+'.pdf','application/pdf');app.notify('PDF DPO creato','Scaricato il fascicolo dettagliato per la consegna. Custodire la copia, verificarla e acquisirla agli atti.','success');return;}
 ...
 function clear(){app=undefined;const host=document.getElementById('privacy-review-dialog');if(host){if(host.open)host.close();host.remove();}}
