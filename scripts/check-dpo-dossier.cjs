...
let blobs=[],notifications=[],failResource=false,denyMaterials=false;
const materials={version:definition.VERSION,fields:definition.fields,sections:definition.sections,release:{commit:'a'.repeat(40)},inventory:fs.readFileSync('docs/inventario-release.json','utf8'),development:'{"scope":"SYNTHETIC EXPORT TEST"}',procedure:fs.readFileSync('docs/19_FASCICOLO_DPO_E_RIPRESA.md','utf8')};
...
const ui=context.window.LeviPrivacyReview;ui.configure({api:{getVotingReview:async()=>({data:review}),getDpoReviewProfile:async()=>({data:profile}),getDpoDossierMaterials:async()=>{if(denyMaterials)throw new Error('Ruolo non autorizzato');return {data:materials};},getVotingReviewEvents:async()=>({data:{snapshotUntil:data.technical.snapshotUntil,events:[],nextCursor:null}})},logs:async()=>data.technical,config:()=>({annoScolastico:'2026/2027'}),notify:(...n)=>notifications.push(n)});
(async()=>{
 ...
 await ui.exportDossier(button,'pdf');assert.equal(blobs.length,2);assert.equal(blobs[1].type,'application/pdf');assert.equal(Buffer.from(await blobs[1].arrayBuffer()).subarray(0,4).toString(),'%PDF');
 denyMaterials=true;await ui.exportDossier(button,'pdf');assert.equal(blobs.length,2);denyMaterials=false;
 failResource=true;await ui.exportDossier(button);assert.equal(blobs.length,2,'missing file must not produce a partial archive');assert.equal(notifications.at(-1)[2],'error');
 ...
 console.log('PASS: actual direct PDF and ZIP, authorization failure yields no download, ten entries with matching SHA-256, missing evidence remains unverified, interrupted export creates no archive.');
