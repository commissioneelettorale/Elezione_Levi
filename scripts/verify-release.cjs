...
fs.writeFileSync('docs/verifiche-sviluppo.json',JSON.stringify({format:'LEVI_DEVELOPMENT_EVIDENCE_V2',generatedAt:new Date().toISOString(),evidence,source,notVerified:['Capacità e tempi di risposta con oltre 1500 utenti reali simultanei su Vercel/Firestore; richiede ambiente cloud di collaudo isolato e monitoraggio dei provider.','Login Production con credenziali reali, dispositivi della scuola e storage dei servizi remoti; il test browser registrato usa risposte Firebase/API fittizie.','Firebase Rules distribuite, IAM/MFA e log dei provider Production.','Backup, ripristino reale, custodi e conservazione scolastica.','Distribuzione fisica dei codici, non correlabilità dell’intera infrastruttura e verifica indipendente.','Validità degli atti, nomine, DPIA e parere DPO.'],privacy:'Nessun accesso a voti reali, creazione di credenziali reali, migrazione o apertura elettorale durante queste prove.'},null,2)+'\n');
